...
def validate_list(value, name, valid_options=None, min_length=None, max_length=None):
    """Validate list parameter with optional constraints.
    ...
    valid_options : list, optional
        List of valid values that elements must be in
    ...
    ValueError
        If value doesn't meet length constraints or contains invalid elements

    Examples
    --------
    >>> validate_list(['mean', 'std'], 'methods', valid_options=['mean', 'std', 'skewness'])
    ['mean', 'std']
    >>> validate_list('mean', 'methods')
    TypeError: methods must be a list, got str
    >>> validate_list(['invalid'], 'methods', valid_options=['mean', 'std'])
    ValueError: methods contains invalid elements: ['invalid']. Valid options: ['mean', 'std']
    """
    ...
    if valid_options is not None:
        invalid_elements = [item for item in value if item not in valid_options]
        if invalid_elements:
            raise ValueError(f"{name} contains invalid elements: {invalid_elements}. Valid options: {valid_options}")
